# ci-checks Specification

## Purpose
Automated verification — formatting, linting, type-correctness, tests, and a successful build — that runs on every pull request to `main` and blocks merges that do not pass, with `main` reachable only through pull requests. The gate is defined once and reused by the release pipeline, so "PR checks" and "release checks" cannot drift. Because the repository is public, these checks run without the ability to publish.
