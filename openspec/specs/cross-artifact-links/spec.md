# cross-artifact-links Specification

## Purpose
Turning references to known capabilities, changes, and archived changes inside rendered documents into navigation to those artifacts. References resolve only against ids present in the project's live snapshot, so an unknown name can never be linked — false positives are impossible by construction rather than by tuning — and linkability follows the project's live state as changes are archived and capabilities appear.
