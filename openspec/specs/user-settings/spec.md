...
## Purpose
Durable per-user preferences for the viewer: persisted to a file in an OS-appropriate user configuration directory, viewed and changed on a dedicated settings page reachable from the app shell, tolerant of a missing or malformed file, and confined to the user config directory so a project's `openspec/` state is never written. The first preference is the default artifact tab applied when a change drill-in opens.
