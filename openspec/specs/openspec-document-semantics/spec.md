# openspec-document-semantics Specification

## Purpose
Rendering OpenSpec's own document structure — requirements, scenarios, WHEN/THEN steps, the normative `SHALL`, and delta operation groupings — as recognizable, consistently-styled units on every markdown surface. Operations share one visual vocabulary with the diff view, so an operation means the same thing wherever it is met. Structure is detected, never required: a document without it renders as ordinary markdown. This capability governs *meaning*; `markdown-rendering` governs the underlying document rendering.
