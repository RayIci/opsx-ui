# change-artifact-nav Specification

## Purpose
A consistent, switchable navigation over a change's artifacts — Proposal, Design, Tasks, and Spec changes — used by both the active-change drill-in and the archived-change view, so a change is read the same way whether it is live or archived. Absent artifacts are shown as disabled destinations, Proposal is the default, and a change directory's `README.md` is not a destination.
